  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['reportType'])) {
    ...
    switch ($reportType) {
      case 'report1':
        $query = "SELECT 
                  ...
                  COUNT(CASE WHEN o.opType = 'Выдача' THEN 1 END) AS КоличествоВыдач,
                  COUNT(CASE WHEN o.opType = 'Возврат' THEN 1 END) AS КоличествоВозвратов,
                  COUNT(CASE WHEN o.opType = 'Списание' THEN 1 END) AS КоличествоСписаний,
    ...
    if (!empty($query)) {
      ...
      if ($result->rowCount() > 0) {
        $reportData = "<table class='table table-bordered'><thead><tr>";
        ...
        while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
          ...
          foreach ($row as $value) {
            $reportData .= "<td>" . htmlspecialchars($value) . "</td>";
          }
          $reportData .= "</tr>";
        }
